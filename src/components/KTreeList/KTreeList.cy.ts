...
describe('KTreeList', () => {
  ...
  it('Children list should be collapsed onclick a caret item', () => {
    ...
    cy.getTestId(`tree-item-wrapper-${patentIds[0]}`).findTestId('tree-item-expanded-icon').trigger('click', { force: true })
  ...
  it('Caret icon should not exist if `collapsible` prop is set to `false`', () => {
    ...
    cy.getTestId('tree-item-id1').findTestId('tree-item-expanded-icon').should('not.exist')
  })
})
