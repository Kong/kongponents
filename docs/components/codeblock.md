...
::: warning
The returned string is rendered via `v-html`. Make sure the HTML is either generated by a trusted library (e.g. a syntax highlighter) or properly escaped to prevent XSS.
:::
...
function highlight({ code, language }) {
  if (!Prism.languages[language]) {
    ...
    // Escape HTML to avoid XSS — the returned string is rendered via v-html
